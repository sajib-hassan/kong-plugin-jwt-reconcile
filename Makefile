...
KONG_PATH ?=/kong
PLUGIN_NAME := kong-plugin-jwt-reconcile
...
install:
	sudo luarocks --lua-dir=/usr/local/opt/lua@5.1 make $(PLUGIN_NAME)-*.rockspec

uninstall:
	sudo luarocks --lua-dir=/usr/local/opt/lua@5.1 remove $(PLUGIN_NAME)-*.rockspec

install-dev:
	luarocks --lua-dir=/usr/local/opt/lua@5.1 make --tree lua_modules $(PLUGIN_NAME)-*.rockspec
